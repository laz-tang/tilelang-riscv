...
def test_gqa_bwd_float32_runtime_compare():
    batch, heads, heads_kv, seq_len, dim = 1, 1, 1, 4, 4
    is_causal = False

    q = torch.linspace(-0.4, 0.5, batch * seq_len * heads * dim,
                       dtype=torch.float32).reshape(batch, seq_len, heads, dim)
    k = torch.linspace(0.2, -0.3, batch * seq_len * heads_kv * dim,
                       dtype=torch.float32).reshape(batch, seq_len, heads_kv, dim)
    v = torch.linspace(-0.7, 0.6, batch * seq_len * heads_kv * dim,
                       dtype=torch.float32).reshape(batch, seq_len, heads_kv, dim)
