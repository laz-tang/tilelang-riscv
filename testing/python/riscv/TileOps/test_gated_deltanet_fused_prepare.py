...
def _load_gated_prepare_module():
    _ensure_minimal_tileops_kernel_modules()
...
def test_gated_deltanet_fused_prepare_compute_w_u_float32_runtime_compare():
    batch, heads, seq_len, chunk_size = 1, 1, 2, 2
    dim_k = dim_v = 16
    module = _load_gated_prepare_module()
    ...
    actual_aw, actual_au, actual_w, actual_u = kernel(k, v, g, beta)
    ...
    for i in range(chunk_size):
        for j in range(chunk_size):
            if i > j:
                transform[i, j] = -gram[i, j] * beta[0, 0, i] * torch.exp(g[0, 0, i] - g[0, 0, j])
