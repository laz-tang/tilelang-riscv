...
# Original TileKernels source: tile_kernels/quant/per_channel_cast_kernel.py
def test_per_channel_cast_kernel_runtime_compare():
    x = (torch.arange(128 * 128, dtype=torch.float32).reshape(128, 128) / 128 - 3).to(torch.bfloat16).contiguous()
    ...
    in_config = CastInputConfig(torch_dtype=torch.bfloat16, with_sf=False, sf_block=(1, 1))
