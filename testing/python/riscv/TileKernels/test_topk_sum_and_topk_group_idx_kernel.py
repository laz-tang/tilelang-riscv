...
def test_topk_sum_and_topk_group_idx_kernel_runtime_compare():
    scores = torch.tensor(
        [
            [[9.0, 1.0, 0.0, 0.0], [8.0, 2.0, 0.0, 0.0], [7.0, 3.0, 0.0, 0.0], [6.0, 4.0, 0.0, 0.0]],
            [[1.0, 9.0, 0.0, 0.0], [2.0, 8.0, 0.0, 0.0], [3.0, 7.0, 0.0, 0.0], [4.0, 6.0, 0.0, 0.0]],
        ],
        dtype=torch.float32,
    )
    ...
    kernel = tilelang.compile(
        get_topk_sum_and_topk_group_idx_kernel.get_tir(4, 4, 2, 2),
        out_idx=[1],
        target="riscv",
    )
    try:
